...
namespace Barber.AutoDiagrammer.Services
{
	...
	public class TreeCreator : ITreeCreator
	{
		...
		public List<AssemblyTreeViewModel> ScanAssemblyAndCreateTree(string assemblyFileName, ITypeFilter typeFilter)
		{
			...
			try
			{
				tree = ScanAssemblyAndCreateTree(assembly, typeFilter);
			}
			catch
			{
				tree = null;
		...
		internal List<AssemblyTreeViewModel> ScanAssemblyAndCreateTree(Assembly assembly, ITypeFilter typeFilter)
		{
			try
			{
				AppDomain.CurrentDomain.AssemblyResolve += ReflectionOnlyResolveEventHandler;
				List<AssemblyTreeViewModel> tree = GroupAndCreateTree(assembly, typeFilter);
				return tree;
			}
			finally
			{
		...
		private List<AssemblyTreeViewModel> GroupAndCreateTree(Assembly assembly, ITypeFilter typeFilter)
		{
		...
		{
			TypeReflector.RequiredBindings = BindingFlags.Public;
			TypeReflector.ShowConstructorParameters = true;
			TypeReflector.ShowFieldTypes = true;
			TypeReflector.ShowPropertyTypes = true;
			TypeReflector.ShowInterfaces = true;
			TypeReflector.ParseMethodBodyIL = false;
			TypeReflector.ShowMethodArguments = true;
			TypeReflector.ShowMethodReturnValues = true;
			TypeReflector.ShowGetMethodForProperty = false;
			TypeReflector.ShowSetMethodForProperty = false;
			TypeReflector.ShowEvents = false;
			TypeReflector.IncludeConstructorParametersAsAssociations = true;
			TypeReflector.IncludePropertyTypesAsAssociations = true;
			TypeReflector.IncludeFieldTypesAsAssociations = true;
			TypeReflector.IncludeMethodArgumentAsAssociations = true;
